...
pub fn validate_packet_attributes(packet_attributes: PacketAttributes) -> syn::Result<(i32, Ident, Ident, i32)> {
...
pub fn packet_macro_impl(input: &DeriveInput) -> syn::Result<proc_macro::TokenStream> {
    ...
    let (id, side, state, protocol) = validate_packet_attributes(packet_attributes)?;
    ...
    let packet_writable_trait = writable_trait_from_body(input, write_body)?;
    let packet_readable_trait = readable_macro_impl(input)?;
